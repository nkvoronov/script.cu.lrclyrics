...
class MAIN():
    def __init__(self, *args, **kwargs):
        self.service = kwargs['mode']
        log('started as service: %s' % str(self.service))
        self.setup_main()
    ...
    def myPlayerChanged(self):
            ...
            if (song and (str(self.current_lyrics.song) != str(song))):
                ...
                if lyrics.lyrics:
                    ...
                    if xbmc.getCondVisibility('Window.IsVisible(12006)') and not WIN.getProperty('culrc.guirunning') == 'TRUE':
                        ...
                        gui = guiThread(mode=self.service, save=self.save_lyrics_to_file, remove=self.remove_lyrics_from_memory, delete=self.delete_lyrics, function=self.return_time)
                        gui.start()
                else:
        ...
        if xbmc.getCondVisibility('MusicPlayer.HasNext') and songchanged:
            ...
            if next_song:
                ...
                log('Missing Artist or Song name for next track')
    ...
    def update_settings(self):
        self.get_scraper_list()
        self.service = ADDON.getSettingBool('service')
        if not self.service:
...
class guiThread(threading.Thread):
    ...
    def run(self):
        ui = GUI('script-cu-lrclyrics-main.xml', CWD, 'Default', mode=self.service, save=self.save, remove=self.remove, delete=self.delete, function=self.function)
        ui.doModal()
        del ui
...
class GUI(xbmcgui.WindowXMLDialog):
    ...
    def exit_gui(self, action):
        ...
        if (not self.service) and (action == 'quit'):
